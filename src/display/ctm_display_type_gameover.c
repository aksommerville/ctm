...
#define CTM_GAMEOVER_MINW  768 // logo==256, allow at least the same amount left and right for statistics.
#define CTM_GAMEOVER_MINH  300 // (32*9==288), plus some margin
...
/* Draw scatter chart (one little dot for each voter showing party and sentiment).
 */
...
static int ctm_display_gameover_draw_summary_bar(struct ctm_display *display,int x,int y,int w,int h,int stateix,const struct ctm_election_result *result) {
  ...
  if (total>0) {
    ...
    if (ctm_video_add_textf(x+8,y+(h>>1),0xffffffff,"%d%%",bluepct)<0) return -1;
    int redx=x+w-8;
    if (redpct>=100) redx-=24;
    else if (redpct>=10) redx-=16;
    else redx-=8;
...
static int ctm_display_gameover_draw_nationwide(struct ctm_display *display,int limitx,int limity,int limitw,int limith,const struct ctm_election_result *result) {
  ...
  int contenth=124;
  if (ctm_game.playerc>=3) contenth+=64; // Two rows of player results.
  else contenth+=32; // One row of player results.
  contenth+=16*4; // spacer, timelimit, population
  if (had_werewolf) contenth+=16; // difficulty
  if (ctm_game.time_remaining.min||ctm_game.time_remaining.s||ctm_game.time_remaining.cs) contenth+=16; // cancellation message
  if ((result->elecblue>result->elecred)&&(result->totalblue<result->totalred)) contenth+=16; // popular vs electoral message
  else if ((result->elecblue<result->elecred)&&(result->totalblue>result->totalred)) contenth+=16; // "
  else if (result->totalblue&&(result->totalblue==result->totalred)) contenth+=16; // popular vote tie
  else if (!result->totalblue||!result->totalred) contenth+=16; // popular sweep
  ...
  // Election results.
  if (ctm_display_gameover_draw_national_map(display,midx-56,topy,52,52,result)<0) return -1;
  if (ctm_display_gameover_draw_demographics(display,midx+4,topy,52,52,&ctm_group_voter)<0) return -1;
  if (ctm_display_gameover_draw_scatter_bar(display,limitx+32,topy+68,limitw-64,16,&ctm_group_voter)<0) return -1;
  if (ctm_display_gameover_draw_summary_bar(display,limitx+32,topy+92,limitw-64,16,-1,result)<0) return -1;

  // Draw results for each player and the werewolf.
  int y=topy+124;
  if (had_werewolf) switch (ctm_game.playerc) {
    case 1: {
        if (ctm_display_gameover_draw_player_result(display,midx-36,y,1,result)<0) return -1;
        if (ctm_display_gameover_draw_player_result(display,midx+4,y,0,result)<0) return -1;
        y+=48;
      } break;
    case 2: {
        if (ctm_display_gameover_draw_player_result(display,midx-52,y,1,result)<0) return -1;
        if (ctm_display_gameover_draw_player_result(display,midx-20,y,2,result)<0) return -1;
        if (ctm_display_gameover_draw_player_result(display,midx+20,y,0,result)<0) return -1;
        y+=48;
      } break;
    case 3: {
        if (ctm_display_gameover_draw_player_result(display,midx-52,y,1,result)<0) return -1;
        if (ctm_display_gameover_draw_player_result(display,midx-20,y,2,result)<0) return -1;
        if (ctm_display_gameover_draw_player_result(display,midx-52,y+32,3,result)<0) return -1;
        if (ctm_display_gameover_draw_player_result(display,midx+20,y+16,0,result)<0) return -1;
        y+=80;
      } break;
    case 4: {
        if (ctm_display_gameover_draw_player_result(display,midx-52,y,1,result)<0) return -1;
        if (ctm_display_gameover_draw_player_result(display,midx-20,y,2,result)<0) return -1;
        if (ctm_display_gameover_draw_player_result(display,midx-52,y+32,3,result)<0) return -1;
        if (ctm_display_gameover_draw_player_result(display,midx-20,y+32,4,result)<0) return -1;
        if (ctm_display_gameover_draw_player_result(display,midx+20,y+16,0,result)<0) return -1;
        y+=80;
      } break;
  } else switch (ctm_game.playerc) {
    case 1: { // This mode doesn't exist, but whatever.
        if (ctm_display_gameover_draw_player_result(display,midx-16,y,1,result)<0) return -1;
        y+=48;
      } break;
    case 2: {
        if (ctm_display_gameover_draw_player_result(display,midx-32,y,1,result)<0) return -1;
    ...
    case 3: {
        if (ctm_display_gameover_draw_player_result(display,midx-32,y,1,result)<0) return -1;
        if (ctm_display_gameover_draw_player_result(display,midx,y,2,result)<0) return -1;
        if (ctm_display_gameover_draw_player_result(display,midx-32,y+32,3,result)<0) return -1;
        y+=80;
      } break;
    case 4: {
        if (ctm_display_gameover_draw_player_result(display,midx-32,y,1,result)<0) return -1;
        if (ctm_display_gameover_draw_player_result(display,midx,y,2,result)<0) return -1;
        if (ctm_display_gameover_draw_player_result(display,midx-32,y+32,3,result)<0) return -1;
        if (ctm_display_gameover_draw_player_result(display,midx,y+32,4,result)<0) return -1;
        y+=80;
      } break;
  }
  ...
  int textx=limitx+16;
  if (ctm_video_begin_text(16)<0) return -1;
  if (ctm_video_add_textf(textx,y+8,0xffffffff,"Duration: %d:00",ctm_game.timelimit_minutes)<0) return -1; y+=16;
  if (ctm_game.time_remaining.min||ctm_game.time_remaining.s||ctm_game.time_remaining.cs) {
    if (ctm_video_add_textf(textx,y+8,0xffff00ff,"Aborted at %d:%02d.",ctm_game.time_remaining.min,ctm_game.time_remaining.s)<0) return -1;
    y+=16;
  }
  if (ctm_video_add_textf(textx,y+8,0xffffffff,"Population: %d",ctm_game.population)<0) return -1; y+=16;
  if (had_werewolf) {
    const char *diffname="?";
    ...
    if (ctm_video_add_textf(textx,y+8,0xffffffff,"Difficulty: %s",diffname)<0) return -1;
    y+=16;
  }
  if ((result->elecblue>result->elecred)&&(result->totalblue<result->totalred)) {
    if (ctm_video_add_textf(textx,y+8,0xffff00ff,"Red won the popular vote.")<0) return -1; y+=16;
  } else if ((result->elecblue<result->elecred)&&(result->totalblue>result->totalred)) {
    if (ctm_video_add_textf(textx,y+8,0xffff00ff,"Blue won the popular vote.")<0) return -1; y+=16;
  } else if (result->totalblue&&(result->totalblue==result->totalred)) {
    if (ctm_video_add_textf(textx,y+8,0xffff00ff,"Popular vote tied at %d.",result->totalblue)<0) return -1; y+=16;
  } else if (!result->totalblue||!result->totalred) {
    if (ctm_video_add_textf(textx,y+8,0xffff00ff,"Popular vote unanimous!")<0) return -1; y+=16;
  }
  if (ctm_video_end_text(ctm_video.texid_font)<0) return -1;
...
static int ctm_display_gameover_draw_perstate(struct ctm_display *display,int limitx,int limity,int limitw,int limith,const struct ctm_election_result *result) {
  int rowh=33; // includes 1 pixel margin on the bottom
  int contenty=limity+(limith>>1)-((rowh*9)>>1);
  int contentw=limitw-20; // 20 pixels for horizontal margins
  if (contentw<74) return 0; // sanity check, need 64 pixels plus a bit more for the graphs.
...
static int ctm_display_gameover_redraw_1(struct ctm_display *display) {
  ...
  int logox=(display->fbw>>1)-128;
  int logoy=(display->fbh>>1)-128;
  ...
  if (ctm_draw_texture(logox,logoy,256,256,logoid,0)<0) return -1;
  ...
  if (ctm_display_gameover_draw_perstate(display,logox+256,0,display->fbw-logox-256,display->fbh,&result)<0) return -1;

  // Draw bunting in both upper corners. The image is 64x64, native to left side.
  if (ctm_draw_texture(0,0,64,64,ctm_video.texid_bunting,0)<0) return -1;
  if (ctm_draw_texture_flop(display->fbw-64,0,64,64,ctm_video.texid_bunting,0)<0) return -1;
...
static int _ctm_display_gameover_draw(struct ctm_display *display) {
  ...
  if (++(DISPLAY->counter)>=120) {
    if ((DISPLAY->counter-120)%60<50) {
      if (ctm_video_begin_text(16)<0) return -1;
      if (ctm_video_add_textf_centered(0,display->h-50,display->w,50,0xffff00ff,"Press any key to restart.")<0) return -1;
      if (ctm_video_end_text(ctm_video.texid_font)<0) return -1;
    }
  }
